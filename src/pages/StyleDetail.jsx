...
export default function StyleDetail({ embedded = false, hideInfo = embedded }) {
  ...
  const renderSummary = () => (
    <div className="sc-summary">
      ...
      {!hideInfo && (
        <>
          <h3 className="sc-section-title">검사 요청 정보</h3>
          <table className="sc-info-table">
            <tbody>
              ...
              <tr><th>이름</th><td>{CHECK_INFO.userName}</td><th>출처 표기법</th><td>{CHECK_INFO.citationStyle}</td></tr>
              <tr><th>검사 완료일</th><td>{CHECK_INFO.completedAt}</td><th>검사 범위</th><td>{CHECK_INFO.checkScope}</td></tr>
            </tbody>
          </table>
        </>
      )}
  ...
  return (
    <div className="sc-page">
      ...
      <div className="sc-file-header">
        <span className="sc-check-type-badge">스타일 검사</span>
