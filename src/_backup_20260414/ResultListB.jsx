  ...
  return (
    <div className="rl-page">
      ...
      <div className="rl-title-row">
        <h2 className="rl-title">참고문헌 검증 <span className="rl-variant-badge">{variant}</span></h2>
      ...
      <div className="rl-info-banner">
        ...
        <span>
          참고문헌 검증은 문서에 기재된 출처의 <strong>실재 여부</strong>와 <strong>인용 규격의 정확성</strong>을 정밀 진단하는 서비스입니다.<br />
          '참고문헌 진위 검증'으로 가짜 문헌 리스크를 차단하고, '인용 적절성 검증'으로 학술적 완성도를 높여보세요.
        </span>
      </div>
      ...
      <div className="rl-table-wrap">
        <table className="rl-table rl-table-b">
          <thead>
            <tr>
              ...
              <th className="col-validity-b">진위성 검증</th>
              <th className="col-style-b">적절성 검증</th>
                ...
                <tr key={row.id}>
                  ...
                  <td className="col-validity-b">
                    {row.validity === 'unscanned' ? (
                      ...
                      <div className="rlb-risk-item validity">
                        <span className="rlb-risk-label">미확인(허위의심)</span>
                        <span className="rlb-risk-count">{row.validity.unconfirmed}<span className="rlb-risk-unit">건</span></span>
                      </div>
                    ) : (
                      <span className="rl-dash">-</span>
                    )}
                  </td>
                  <td className="col-style-b">
                    {row.style === 'skipped' ? (
                      <span className="rl-dash">미실시</span>
                    ) : row.status === 'done' ? (
                      <div className="rlb-risk-item style">
                        <span className="rlb-risk-label">작성오류/품질주의</span>
                        <span className="rlb-risk-count">{row.style.risk}<span className="rlb-risk-unit">건</span></span>
                      </div>
                    ) : (
                      <span className="rl-dash">-</span>
                    )}
                  ...
                  <td className="col-action">
                    {row.status === 'done' && row.style !== 'skipped' && row.validity !== 'unscanned' ? (
                      <Link to={`${prefix}/detail/${row.id}`} className="rlb-detail-btn">
                        결과 확인
                      </Link>
                    ) : row.status === 'done' ? (
                      <Link to={`${prefix}/detail/${row.id}`} className="rlb-detail-btn">
